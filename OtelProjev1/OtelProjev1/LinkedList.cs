...
namespace OtelProjev1
{
    public class LinkedList : LinkedListADT
    {
        ...
        public LinkedList OteleGoreCalisan(int OtelID)
        {
            Node tempNode = Head;
            LinkedList otelYorumlari = new LinkedList();

            while (tempNode != null)
            {
                if ((tempNode.Data as Calisan).CalistigiOtel == OtelID)
                {
                    otelYorumlari.InsertFirst((tempNode.Data as Calisan));
